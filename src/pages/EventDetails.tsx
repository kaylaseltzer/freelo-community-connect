...
import { CalendarDays, Clock, MapPin, Heart, Share2, Ticket, ExternalLink } from 'lucide-react';
...
const EventDetails: React.FC = () => {
  ...
  const handleSignUp = () => {
    ...
    // For now, we'll just show an info toast
    if (event?.ticketLink) {
  ...
  // Add mock ticket link for demo purposes
  // In a real app, this would come from the database
  const ticketLink = event.ticketLink || "https://example.com/tickets";
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
            <Card>
              <CardContent className="pt-6">
                ...
                <div className="space-y-4">
                  <div>
                    <h3 className="font-medium text-muted-foreground">{t('event.attendeeLimit')}</h3>
                    <p>{event.attendeeLimit || "No limit"}</p>
                  </div>
                  <div>
                    <h3 className="font-medium text-muted-foreground">{t('event.ageRestriction')}</h3>
                    <p>{event.ageRange || "No age restriction"}</p>
                  </div>
                  <div>
                    <h3 className="font-medium text-muted-foreground">{t('event.accessMode')}</h3>
            ...
            <Card>
              <CardContent className="pt-6">
                ...
                <div className="space-y-4">
                  <div>
                    <h3 className="font-medium text-muted-foreground">{t('event.host')}</h3>
                    <p>{event.host || "Anonymous"}</p>
                  </div>
                  <div>
                    <h3 className="font-medium text-muted-foreground">{t('event.contact')}</h3>
                    <p>{event.contact || "Contact information not provided"}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
